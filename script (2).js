...
(function () {
  ...
  function photoPlate(item) {
    ...
    item.images.forEach(function (src, i) {
      ...
      img.alt = item.alt
        ? item.alt
        : (item.title || item.who || item.question || "Family photograph") +
  ...
  function voiceScene(item, cat, code) {
    ...
    var portrait = photoPlate(item);
    if (portrait) {
      portrait.classList.add("memory__plate--voice");
      s._body.appendChild(portrait);
    } else {
      s.classList.add("voice--textonly");
    }
  ...
  /* On iOS the address bar collapses as you scroll, which changes
     window.innerHeight by ~60-90px mid-gesture. Recomputing the centre of the
     screen from that makes every memory jump in brightness at once. So we hold
     a stable height and only refresh it on a real resize or rotation.        */
  var vhRef = window.innerHeight;
  var vwRef = window.innerWidth;

  function refreshViewport(force) {
    var h = window.innerHeight, w = window.innerWidth;
    if (force || w !== vwRef || Math.abs(h - vhRef) > 160) {
      vhRef = h;
      vwRef = w;
    }
  }

  /* Writing --f on every pixel of scroll repaints far more than the eye can
     see. Rounding to a step means most frames write nothing at all.          */
  var STEP = coarse.matches ? 0.04 : 0.02;
  ...
  function update() {
    ...
    var vh = vhRef;
    var centre = vh / 2;
    var span_ = vh * FOCUS_SPAN;      // how far from centre a scene can glow
    var best = null, bestF = -1;

    /* read every position FIRST, then write. Interleaving reads and writes
       forces the browser to re-do layout once per element.                   */
    var reads = [];
    live.forEach(function (s) {
      reads.push([s, s.getBoundingClientRect()]);
    });
    var stageTop = stage.getBoundingClientRect().top;
    var stageSpan = stage.offsetHeight - vh;

    for (var i = 0; i < reads.length; i++) {
      var s = reads[i][0], r = reads[i][1];
      var c = r.top + r.height / 2;
      var f = clamp(1 - Math.abs(c - centre) / span_, 0, 1);
      f = f * f * (3 - 2 * f);         // smoothstep
      if (f > bestF) { bestF = f; best = s; }

      var q = Math.round(f / STEP) * STEP;
      if (s._f !== q) {
        s._f = q;
        s.style.setProperty("--f", q.toFixed(2));
      }
    }

    if (best && best.dataset.cat) setActive(best.dataset.cat);

    railFill.style.setProperty("--p",
      String(stageSpan > 0 ? clamp(-stageTop / stageSpan, 0, 1) : 0));
  }
  ...
  window.addEventListener("orientationchange", function () {
    setTimeout(function () { refreshViewport(true); requestUpdate(); }, 320);
  });
